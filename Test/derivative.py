...
def DerivativeSignal():
    ...
    # claculate second drivative
    SecondDrev = []
    for i in range(len(InputSignal)-1):
        if i != 0:
            calc = InputSignal[i + 1] - 2 * InputSignal[i] + InputSignal[i-1]
            SecondDrev.append(calc)

    firstIdicies = range(len(FirstDrev))
    ...
    if( (len(FirstDrev)!=len(expectedOutput_first)) or (len(SecondDrev)!=len(expectedOutput_second))):
        # print(len(FirstDrev),"first")
        # print(len(expectedOutput_first), "expected")
        # print(len(SecondDrev),"Second")
        # print(len(expectedOutput_second),"expected")
    ...
    Task6.plot(firstIdicies,FirstDrev,SecondDrev,"")
